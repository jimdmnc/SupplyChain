...
if (birthdayInput && ageDisplay && ageInput) {
  birthdayInput.addEventListener("change", function () {
    const value = this.value // Expected: "YYYY/MM/DD"

    // Split and validate
    const [year, month, day] = value.split("/")
    if (!year || !month || !day) {
      console.warn("Invalid birthday format")
  ...
  if (step2NextBtn) {
    ...
    const govIdFile = document.getElementById('govIdFile').files[0];

    if (!govIdType) {
      alert('Please select a government issued ID type.');
      document.getElementById('govIdType').focus();
      return;
    }

    if (!govIdFile) {
      alert('Please upload a photo of your selected government ID.');
      document.getElementById('govIdFile').focus();
      return;
    }
...
emailInput.addEventListener("input", function () {
  ...
  if (!isValidFormat) {
    ...
    emailInput.classList.add("is-invalid")
    emailInput.classList.remove("is-valid")
    emailFeedback.textContent = "This domain is not currently supported."
    emailFeedback.style.display = "block"
    emailValidFeedback.style.display = "none"
  } else {
  ...
  if (retailerForm) {
    retailerForm.addEventListener("submit", (e) => {
      ...
      fetch("register_retailer.php", {
       ...
       const alertContainer = document.getElementById("registration-alert-container");
