  ...
  if (approvalModal) {
    approvalModal.addEventListener("show.bs.modal", () => {
      ...
      if (messageArea && currentRetailerForApproval) {
        messageArea.value = `Dear ${
          currentRetailerForApproval.first_name || "Applicant"
        },
...
function showApprovalModal(retailer) {
  ...
  document.getElementById(
    "approvalSubject"
  ).value = `Congratulations! Your Retailer Application has been Approved - Pinana Gourmet`;
...
function handleSendApprovalEmail() {
  ...
  const emailData = {
    ...
    subject: subject,
    message: message,
    retailer_name: `${currentRetailerForApproval.first_name || ""} ${
      currentRetailerForApproval.last_name || ""
    }`.trim(),
  };
  ...
  fetch("send_approved_email.php", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        const approvalModal = bootstrap.Modal.getInstance(
          document.getElementById("approvalModal")
        );
        approvalModal.hide();

        // Close retailer details modal if open
        const retailerDetailsModal = bootstrap.Modal.getInstance(
          document.getElementById("retailerDetailsModal")
        );
        ...
        showAlert(
          "success",
          "Approval email sent successfully and retailer status updated to approved."
        );
        ...
        showAlert(
          "danger",
          "Failed to send approval email: " + (data.message || "Unknown error")
        );
      }
    })
    .catch((error) => {
      ...
      if (error instanceof TypeError) {
        errorMessage =
          "Network error or server unreachable. Please check your internet connection and server status.";
  ...
  if (rejectionModal) {
    rejectionModal.addEventListener("show.bs.modal", () => {
      ...
      if (messageArea && currentRetailerForRejection) {
        messageArea.value = `Dear ${
          currentRetailerForRejection.first_name || "Applicant"
        },
...
function showRejectionModal(retailer) {
  ...
  document.getElementById("rejectionFromEmail").value =
    "wantargaryen@gmail.com";
  document.getElementById(
    "rejectionSubject"
  ).value = `Application Status Update - Pinana Gourmet`;
...
function handleSendRejectionEmail() {
  ...
  const emailData = {
    ...
    subject: subject,
    message: message,
    retailer_name: `${currentRetailerForRejection.first_name || ""} ${
      currentRetailerForRejection.last_name || ""
    }`.trim(),
  };
  ...
  fetch("send_rejection_email.php", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        showAlert(
          "success",
          "Rejection email sent successfully and retailer status updated to rejected."
        );
        ...
        showAlert(
          "danger",
          "Failed to send rejection email: " + (data.message || "Unknown error")
        );
      }
    })
    .catch((error) => {
...
function filterApprovedRetailers(filterType) {
  ...
  switch (filterType) {
    ...
    case "completed_orders_desc":
      filteredRetailers.sort(
        (a, b) =>
          (Number(b.completed_orders) || 0) - (Number(a.completed_orders) || 0)
      );
      break;
    case "completed_orders_asc":
      filteredRetailers.sort(
        (a, b) =>
          (Number(a.completed_orders) || 0) - (Number(b.completed_orders) || 0)
      );
      break;
    case "revenue_desc":
      filteredRetailers.sort(
    ...
    case "revenue_asc":
      filteredRetailers.sort(
        (a, b) =>
          (Number(a.total_revenue) || 0) - (Number(b.total_revenue) || 0)
      );
      break;
    default:
      filteredRetailers.sort(
  ...
  showAlert(
    "info",
    `Approved retailers sorted by: ${filterLabels[filterType]}`
  );
}
  ...
  fetch("fetch_retailers.php")
    ...
    .then((data) => {
      if (data.success) {
        ...
        console.log(
          "Retailers fetched from fetch_retailers.php:",
          data.retailers
        );
        ...
        const approvedFilter = document.getElementById(
          "approvedRetailersFilter"
        );
        if (approvedFilter) {
        ...
        showAlert(
          "danger",
          "Failed to load retailers: " + (data.message || "Unknown error")
        );
        showErrorState();
      }
    })
...
`;
  ...
  displayRetailers.forEach((retailer) => {
    ...
    <div class="retailer-card-wrapper" style="min-width: 320px; flex-shrink: 0;">
      <div class="retailer-card" onclick="viewRetailerDetails(${
        retailer.user_id
      })">
        <div class="d-flex align-items-start mb-3">
          ...
          <div class="flex-grow-1">
            <h6 class="retailer-name">${retailer.first_name || "N/A"} ${
      retailer.last_name || ""
    }</h6>
        ...
        <div class="order-stats-section mt-3 p-3 bg-light rounded">
          <div class="text-center">
            <div class="row">
              <div class="col-6">
                <div class="stat-number text-success fw-bold h4 mb-1">${
                  retailer.completed_orders || 0
                }</div>
                <div class="stat-label small text-muted">Completed Orders</div>
              </div>
              <div class="col-6">
                <div class="stat-number text-primary fw-bold h5 mb-1">₱${Number(
                  retailer.total_revenue || 0
                ).toLocaleString()}</div>
  ...
  html += `
      ...
      ${
        ...
        <div class="d-flex justify-content-between align-items-center mt-3">
          <span class="text-muted small">Showing 5 of ${
            retailers.length
          } retailers</span>
          <button class="btn btn-outline-primary btn-sm" onclick="showAllRetailers('${status}', ${JSON.stringify(
              retailers
            ).replace(/"/g, "&quot;")})">
...
`;
  ...
  html += `
      ...
      <div class="d-flex justify-content-center mt-3">
        <button class="btn btn-outline-secondary btn-sm" onclick="renderRetailerSection('${status}', ${JSON.stringify(
    retailers
  ).replace(/"/g, "&quot;")})">
...
function updateCounts() {
  document.getElementById("approved-count").textContent =
    currentRetailers.approved.length;
  document.getElementById("pending-count").textContent =
    currentRetailers.pending.length;
  document.getElementById("rejected-count").textContent =
    currentRetailers.rejected.length;
}

// Update statistics cards
function updateStats() {
  document.getElementById("approved-stats").textContent =
    currentRetailers.approved.length;
  document.getElementById("pending-stats").textContent =
    currentRetailers.pending.length;
  document.getElementById("rejected-stats").textContent =
    currentRetailers.rejected.length;
}
...
function viewRetailerDetails(retailerId) {
  ...
  const retailerDetailsContent = document.getElementById(
    "retailerDetailsContent"
  );
  ...
  // Show modal
  const retailerDetailsModal = new bootstrap.Modal(
    document.getElementById("retailerDetailsModal")
  );
...
function renderRetailerActions(retailer) {
  ...
  switch (approvalStatus) {
    case "pending":
      actionsHtml = `
      <button type="button" class="btn btn-approve btn-action" onclick="showApprovalModal(${JSON.stringify(
        retailer
      ).replace(/"/g, "&quot;")})">
        <i class="bi bi-check-circle me-1"></i>Approve Retailer
      </button>
      <button type="button" class="btn btn-reject btn-action" onclick="showRejectionModal(${JSON.stringify(
        retailer
      ).replace(/"/g, "&quot;")})">
    ...
    case "approved":
      actionsHtml = `
      <button type="button" class="btn btn-pending btn-action" onclick="updateRetailerStatus(${
        retailer.user_id
      }, 'pending')">
  ...
  fetch("update_retailer_status.php", {
    ...
    .then((data) => {
      if (data.success) {
        showAlert(
          "success",
          `Retailer status successfully updated to ${newStatus}`
        );
        ...
        showAlert(
          "danger",
          "Failed to update retailer status: " +
...
function renderRetailerDetails(retailer) {
  ...
  // Update modal title
  document.getElementById(
    "retailerDetailsModalLabel"
  ).innerHTML = `<i class="bi bi-person-circle me-2"></i>${
    retailer.first_name || "N/A"
  } ${retailer.last_name || ""}`;

  // Format address
  const address = [
    retailer.house_number,
    retailer.barangay,
    retailer.city,
    retailer.province,
  ]
...
`;
  ...
  // Approval status
  const approvalStatusBadge = getStatusBadge(
  ...
  if (
    ...
    retailer.completed_orders_list.length > 0
  ) {
  ...
  <div class="row">
    <div class="col-md-4">
      <div class="text-center mb-4">
        ...
        <h5 class="mb-1">${retailer.first_name || "N/A"} ${
    retailer.last_name || ""
  }</h5>
      ...
      <div class="card">
        ...
        <div class="card-body text-center">
          ...
          <hr>
          <div>
            <div class="h4 text-primary mb-1">₱${Number(
              retailer.total_revenue || 0
            ).toLocaleString()}</div>
          ...
          <div class="card h-100">
            ...
            <div class="card-body">
              <div class="mb-3">
                ...
                <div class="fw-medium">${retailer.first_name || "N/A"} ${
    retailer.last_name || ""
  }</div>
              </div>
              <div class="mb-3">
...
`
          ...
          <div class="card h-100">
            ...
            <div class="card-body">
              <div class="mb-3">
                ...
                <div class="fw-medium">${
                  retailer.business_name || "Not provided"
                }</div>
              </div>
              <div class="mb-3">
                <label class="form-label text-muted small">Business Type</label>
                <div class="fw-medium">${
                  retailer.business_type || "Not provided"
                }</div>
              </div>
              <div class="mb-3">
              ...
              <div class="mb-3">
                ...
                <div class="fw-medium">${
                  retailer.address_notes || "No additional notes"
                }</div>
              </div>
      ...
      <div class="mb-3">
        ...
        <div class="fw-medium">${
          retailer.business_doc_type || "Not provided"
        }</div>
      </div>
      <div class="mb-0">
              ...
              <div class="row">
                <div class="col-md-6">
                  <div class="mb-3">
                    ...
                    <div class="fw-medium">${
                      retailer.username || "Not available"
                    }</div>
                  </div>
                  <div class="mb-3">
  ...
  fetch(`search_retailers.php?search=${encodeURIComponent(searchTerm)}`)
    ...
    .then((data) => {
      if (data.success) {
        ...
        searchResults.approved.sort(
          (a, b) => new Date(b.created_at) - new Date(a.created_at)
        );
        searchResults.pending.sort(
          (a, b) => new Date(b.created_at) - new Date(a.created_at)
        );
        searchResults.rejected.sort(
          (a, b) => new Date(b.created_at) - new Date(a.created_at)
        );
        ...
        showAlert(
          "danger",
          "Failed to search retailers: " + (data.message || "Unknown error")
        );
        showErrorState();
      }
    })
