...
if ($update_stmt) {
    ...
    if (!mysqli_stmt_execute($update_stmt)) {
        ...
        // -- NEW: After successful update, fetch the new quantity and trigger low-stock email if needed
        $check_sql = "SELECT id, name, quantity FROM raw_materials WHERE id = ?";
        $check_stmt = mysqli_prepare($conn, $check_sql);
        if ($check_stmt) {
            mysqli_stmt_bind_param($check_stmt, 'i', $material_id);
            mysqli_stmt_execute($check_stmt);
            $check_result = mysqli_stmt_get_result($check_stmt);
            $current_material = mysqli_fetch_assoc($check_result);
            mysqli_stmt_close($check_stmt);

            if ($current_material) {
                $new_qty = (int)$current_material['quantity'];
                // threshold -- change 10 to any threshold value you prefer
                $threshold = 10;
                if ($new_qty <= $threshold) {
                    // material name - prefer DB value for accuracy
                    $mat_name = (string)$current_material['name'];

                   $emailResult = sendLowStockEmail($conn, (int)$current_material['id'], $mat_name, $new_qty, /*debug*/ true);
error_log("sendLowStockEmail result for material_id={$material_id}: " . json_encode($emailResult));

// INSERT NOTIFICATION (copy the snippet above here)
if (session_status() !== PHP_SESSION_ACTIVE) {
    session_start();
}
$sessionUserId = $_SESSION['user_id'] ?? 1;
$notifOk = createLowStockNotification($conn, $sessionUserId, (int)$current_material['id'], $mat_name, $new_qty);
error_log("createLowStockNotification result for material_id={$material_id}: " . ($notifOk ? 'ok' : 'failed'));

                }
                
            }
        } else {
            error_log("Failed to prepare quantity check statement: " . mysqli_error($conn));
        }
    }
